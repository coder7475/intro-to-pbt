...
describe("UserList Component", () => {
  ...
  it("should render the correct list of users with fast-check", async () => {
    ...
    fc.assert(
      fc.asyncProperty(
        ...
        async (users) => {
          ...
          await waitFor(() => {
            const elements = screen.getAllByRole("listitem");
            expect(elements).toHaveLength(users.length);
          });
  ...
  it("should render 'No users found' message when no users are present with fast-check", async () => {
    const fc = await import("fast-check");
    fc.assert(
      fc.asyncProperty(
        fc.array(fc.record({ id: fc.integer(), name: fc.string() })),
  ...
  it("should handle network errors with fast-check", async () => {
    const fc = await import("fast-check");
    fc.assert(
      fc.asyncProperty(
        fc.array(fc.record({ id: fc.integer(), name: fc.string() })),
